...
[ADR-0001](docs/adr/ADR-0001-docker-first-development.md) approves local Compose. [ADR-0003](docs/adr/ADR-0003-foundation-runtime.md) records proposed toolchain and [Issue #5](https://github.com/fusumivietnam/SongChart-Next/issues/5) owns the official-starter import. [Docker development runbook](docs/operations/DOCKER_DEVELOPMENT.md) is authoritative for local app/db setup after lockfiles and tests exist. Framework welcome/auth scaffolding is not SongChart approved product UI. [Draft PR #6](https://github.com/fusumivietnam/SongChart-Next/pull/6) contains the starter implementation and lockfiles; do not mark it implemented on `main` before merge or verified without exact-SHA test evidence.
