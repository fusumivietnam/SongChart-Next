...
Docker Compose is the approved local development entry point. The main branch contains the PostgreSQL-only scaffold. [ADR-0003](../adr/ADR-0003-foundation-runtime.md) proposes a scoped app+PostgreSQL 18.6 Docker development runtime and separate PHP-FPM-only production image target. [Draft PR #6](https://github.com/fusumivietnam/SongChart-Next/pull/6) carries the imported application and real lockfiles with development CI evidence; it is not yet on `main`, a production controller, or approved public SongChart UI. The [Docker runbook](../operations/DOCKER_DEVELOPMENT.md) records activation and migration gates. Environment inventory is not a substitute for build or deployment evidence.
