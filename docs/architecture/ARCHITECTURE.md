...
Docker Compose is the approved local development entry point. The main branch contains the PostgreSQL-only scaffold. [ADR-0003](../adr/ADR-0003-foundation-runtime.md) proposes a scoped app+PostgreSQL 18.6 Docker development runtime and separate PHP-FPM-only production image target. This is not a production controller, completed app verification or approved public UI. The [Docker runbook](../operations/DOCKER_DEVELOPMENT.md) records activation and migration gates. Environment inventory is not a substitute for build or deployment evidence.
